...
try {
	...
	switch ($action) {
		...
		case 'debug':
			try {
				// Test de base pour vérifier que l'AJAX fonctionne
				$testData = array(
					'php_version' => PHP_VERSION,
					'curl_available' => function_exists('curl_init'),
					'jeedom_core' => class_exists('ajax'),
					'core_path_used' => $usedCorePath,
				...
				// Tester les fonctions Jeedom de base
				if (function_exists('jeedom::version')) {
					$testData['jeedom_version'] = jeedom::version();
				}
				
				if (function_exists('config::byKey')) {
					$testData['jeedom_config_accessible'] = true;
				}
				
				ajax::success('Test de débogage réussi. Environnement fonctionnel. Détails: ' . json_encode($testData, JSON_PRETTY_PRINT));
			} catch (Exception $e) {
				ajax::error('Erreur dans le test de débogage: ' . $e->getMessage() . ' - Trace: ' . $e->getTraceAsString());
			}
			break;

		case 'testHttpsNoSsl':
			try {
				log::add('hp_printer', 'info', 'Début du test HTTPS sans vérification SSL');
				
				$ipAddress = init('ip_address');
				$protocol = 'https'; // Force HTTPS
				$verifySsl = false; // Force SSL désactivé

				log::add('hp_printer', 'info', 'Test HTTPS forcé - IP: ' . $ipAddress . ', SSL: désactivé');
				...
				log::add('hp_printer', 'info', 'Test HTTPS vers: ' . $url . ' (SSL verify forcé à NON)');
				...
				curl_setopt_array($ch, [
					...
					CURLOPT_SSL_VERIFYPEER => false, // Force désactivé
					CURLOPT_SSL_VERIFYHOST => 0,     // Force désactivé
					CURLOPT_FOLLOWLOCATION => false,
					CURLOPT_USERAGENT => 'Jeedom HP Printer Test HTTPS',
				...
				log::add('hp_printer', 'info', 'Résultat HTTPS - Code HTTP: ' . $httpCode . ', Erreur cURL: ' . $curlErrno . ' (' . $curlError . ')');

				if ($curlErrno) {
					$errorMessage = __('Erreur cURL HTTPS: ', __FILE__) . $curlError . ' (Code: ' . $curlErrno . ')';
					ajax::error($errorMessage);
					return;
				}

				if ($httpCode !== 200) {
					$errorMessage = __('Connexion HTTPS échouée. Code HTTP: ', __FILE__) . $httpCode;
					ajax::error($errorMessage);
					return;
				}

				if (empty($response)) {
					ajax::error(__('Réponse HTTPS vide.', __FILE__));
					return;
				}

				$xml = @simplexml_load_string($response);
				if ($xml === false) {
					ajax::error(__('Réponse HTTPS invalide (XML attendu).', __FILE__));
					return;
				}

				ajax::success(__('Test HTTPS réussi ! L\'imprimante accepte HTTPS sans vérification SSL.', __FILE__));
				
			} catch (Exception $e) {
				ajax::error(__('Erreur test HTTPS: ', __FILE__) . $e->getMessage());
			}
			break;
